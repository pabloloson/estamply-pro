        ...
        <div className="relative px-4 py-8 sm:py-6" style={{ minHeight: shop.banner ? undefined : 'auto' }}>
          ...
          <div className="max-w-5xl mx-auto">
            <div className="flex items-center justify-between">
              ...
              <ShareButton url={typeof window !== 'undefined' ? window.location.href : ''} title={shop.nombre} text={`Mirá el catálogo de ${shop.nombre}`} />
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <button onClick={() => {
              ...
              if (typeof navigator !== 'undefined' && navigator.share) {
                navigator.share({ title: product.name, text: `${product.name} — ${fmt(effectivePrice)} en ${shop.nombre}`, url: productUrl }).catch(() => {})
              } else {
...
function ShareButton({ url, title, text }: { url: string; title: string; text: string }) {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)} className="p-2 rounded-lg bg-white/20 hover:bg-white/30 transition-colors">
        <Share2 size={16} className="text-white" />
      </button>
      {open && (<>
        <div className="fixed inset-0 z-40" onClick={() => setOpen(false)} />
        <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-lg shadow-lg border border-gray-100 z-50 py-1">
          <button onClick={copyLink} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 flex items-center gap-2 text-gray-700">
            {copied ? <Check size={14} className="text-green-500" /> : <Link2 size={14} className="text-gray-400" />}
            {copied ? 'Enlace copiado' : 'Copiar enlace'}
          </button>
          <button onClick={shareWA} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 flex items-center gap-2 text-gray-700">
            <MessageCircle size={14} className="text-green-500" /> WhatsApp
          </button>
          {canShare && (
            <button onClick={shareNative} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 flex items-center gap-2 text-gray-700">
              <Share2 size={14} className="text-gray-400" /> Compartir...
            </button>
          )}
        </div>
      </>)}
