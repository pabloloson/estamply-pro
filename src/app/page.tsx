        ...
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <div className="flex gap-0.5 bg-gray-100 rounded-full p-0.5">
              <button onClick={() => setLang('es')} className={`px-2.5 py-1 rounded-full text-xs font-bold transition ${lang === 'es' ? 'bg-white shadow-sm text-gray-800' : 'text-gray-400'}`}>ES</button>
              <button onClick={() => setLang('pt')} className={`px-2.5 py-1 rounded-full text-xs font-bold transition ${lang === 'pt' ? 'bg-white shadow-sm text-gray-800' : 'text-gray-400'}`}>PT</button>
            </div>
            <Link href="/login" className="text-sm text-gray-500 hover:text-gray-900 transition font-medium">{t.nav.login}</Link>
            <Link href="/signup" className="px-5 py-2.5 rounded-full text-sm font-bold text-white shadow-md hover:shadow-lg transition-all hover:scale-[1.02]" style={{ background: '#6C63FF' }}>{t.nav.cta}</Link>
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-gray-100 bg-white px-4 py-4 space-y-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => { setLang('es'); setMenuOpen(false) }} className={`px-3 py-1 rounded-full text-xs font-bold ${lang === 'es' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-500'}`}>ES</button>
              <button onClick={() => { setLang('pt'); setMenuOpen(false) }} className={`px-3 py-1 rounded-full text-xs font-bold ${lang === 'pt' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-500'}`}>PT</button>
            </div>
            <Link href="/login" className="block text-sm font-medium text-gray-600">{t.nav.login}</Link>
            <Link href="/signup" className="block w-full text-center px-4 py-2.5 rounded-full text-sm font-bold text-white" style={{ background: '#6C63FF' }}>{t.nav.cta}</Link>
          </div>
        )}
      </nav>
            ...
            <div className="flex-1 text-center lg:text-left">
              ...
              <Section delay={200}>
                <div className="mt-8 flex flex-col sm:flex-row items-center lg:items-start gap-3">
                  <Link href="/signup" className="px-8 py-4 rounded-full text-base font-bold text-white shadow-lg hover:shadow-xl transition-all hover:scale-[1.03]" style={{ background: '#6C63FF' }}>
                    {t.hero.cta}
                  </Link>
                </div>
              ...
              <div className="flex-1 flex justify-center">
                ...
                <div className="relative w-[260px]">
                  <div className="rounded-[2.5rem] border-[6px] border-gray-800 bg-gray-800 shadow-2xl overflow-hidden">
                    <div className="w-20 h-5 bg-gray-800 rounded-b-xl mx-auto" /> {/* notch */}
            ...
            {t.pricing.plans.map((plan, i) => (
              <Section key={i} delay={i * 100}>
                <div className={`relative p-6 rounded-2xl bg-white border-2 h-full flex flex-col ${plan.popular ? 'border-purple-400 shadow-xl md:scale-105' : 'border-gray-100'}`}>
                  ...
                  <Link href="/signup" className={`block w-full py-3 rounded-full text-center text-sm font-bold transition-all ${plan.popular ? 'text-white shadow-md hover:shadow-lg' : 'text-gray-700 border border-gray-200 hover:bg-gray-50'}`} style={plan.popular ? { background: '#6C63FF' } : {}}>
                    {t.pricing.cta}
                  </Link>
                </div>
              </Section>
      ...
      <section className="py-16 lg:py-20 relative overflow-hidden">
        ...
        <Section>
          <div className="max-w-3xl mx-auto px-4 text-center">
            ...
            <div className="mt-8">
              <Link href="/signup" className="inline-block px-10 py-4 rounded-full text-base font-bold text-white shadow-lg hover:shadow-xl transition-all hover:scale-[1.03] animate-pulse-subtle" style={{ background: '#6C63FF' }}>
                {t.finalCta.cta}
              </Link>
            </div>
