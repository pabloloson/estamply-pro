  ...
  if (techniques.includes('subli')) {
    ...
    const impresora = await prisma.equipment.create({
      data: {
        ...
        marca: 'Epson', printTimeSec: 65, isDemo: true,
    ...
    const planchaPlana = await prisma.equipment.create({
      data: {
        ...
        cost: 600000, lifespanUses: 20000, tecnicasSlugs: ['subli', 'dtf', 'vinyl'], moneda: 'local', isDemo: true,
      },
    })
    const planchaTazas = await prisma.equipment.create({
      data: {
        userId, name: 'Plancha de tazas', type: 'press_mug', clasificacion: 'plancha',
        cost: 200000, lifespanUses: 2000, tecnicasSlugs: ['subli'], moneda: 'local', isDemo: true,
      },
    })
    const planchaGorras = await prisma.equipment.create({
      data: {
        userId, name: 'Plancha de gorras', type: 'press_cap', clasificacion: 'plancha',
        cost: 30000, lifespanUses: 2000, tecnicasSlugs: ['subli', 'dtf', 'vinyl'], moneda: 'local', isDemo: true,
      },
    })
    ...
    await prisma.product.create({
      data: {
        userId, name: 'Camiseta', baseCost: 10000, category: 'Textil', categoryId: categories['Textil'],
    ...
    await prisma.product.create({
      data: {
        userId, name: 'Taza blanca', baseCost: 3000, category: 'Tazas', categoryId: categories['Tazas'],
        pressEquipmentId: planchaTazas.id, printerEquipmentId: impresora.id,
        timeSubli: 180, moneda: 'local',
        supplierId: suppliers['El rey de las tazas'], isDemo: true,
      },
    })
    await prisma.product.create({
      data: {
        userId, name: 'Gorra trucker', baseCost: 2500, category: 'Gorras', categoryId: categories['Gorras'],
